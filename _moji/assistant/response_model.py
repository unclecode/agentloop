...
class LLMResponse(BaseModel):
    type: Literal['movie_json', 'list_json', 'trailer_json', 'movie_info', 'text_response'] = Field(description="""Type of response: 
        ...
        'list_json': Response for tools `create_favorite_list`, `get_favorite_lists`, `add_to_favorite_list`, `add_to_big_five_list` containing list details in JSON format,
        'movie_info': Response for `answer_movie_question` amd tools When the response contains information answering a movie-related question,
        'text_response': When the tool response is none of other types and it is a string message for the user.
        """
    )
    data: Union[MoviesData, ModifiedListData, MovieTrailerData, MovieInfoData, TextResponseData] = Field(
        description="""Response data, structure depends on type: 
        trailer_json: MovieTrailerData,
        movie_json: MoviesData,
        list_json: ModifiedListData,
        movie_info: MovieInfoData,
        text_response: TextResponseData
        """
    )
    # additional_info: Optional[str] = Field(description="A concise, engaging extra detail or context (max 100 characters)")
    ...
    class Config:
        ...
        #             "answer": "Inception was directed by Christopher Nolan, who also wrote and produced the film. It was released in 2010 and went on to win four Academy Awards.",
        #             "related_movies": [
        #                 {"n": "Inception", "y": 2010, "t": "m", "tmdb_id": "27205"}
        #             ]
        #         },
        #         "additional_info": "Christopher Nolan is known for his complex, mind-bending narratives."
        #     }
        # }
